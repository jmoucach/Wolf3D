...
/*   Updated: 2019/10/12 17:59:12 by jmoucach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
short new_edge_north(t_data *data, t_edge_list **edges, int x, int y)
{
	...
	data->edge_nb++;
	if (*edges == NULL)
		*edges = create_edge_list(16 * x, 16*y, 16 * (x+1), 16 * y, data->edge_nb);
	else
	{
...
short new_edge_south(t_data *data, t_edge_list **edges, int x, int y)
{
	...
	data->edge_nb++;
	if (*edges == NULL)
		*edges = create_edge_list(16 * x, 16*(1+y), 16 * (x+1), 16 * (1+y), data->edge_nb);
	else
	{
		if (!(tmp = (t_edge_list*)malloc(sizeof(t_edge_list))))
			return (0);
...
short new_edge_east(t_data *data, t_edge_list **edges, int x, int y)
{
	...
	data->edge_nb++;
	if (*edges == NULL)
		*edges = create_edge_list(16 * (1+x), 16*y, 16 * (x+1), 16 * (1+y), data->edge_nb);
	else
	{
		if (!(tmp = (t_edge_list*)malloc(sizeof(t_edge_list))))
			return (0);
...
short new_edge_west(t_data *data, t_edge_list **edges, int x, int y)
{
	...
	data->edge_nb++;
	if (*edges== NULL)
		*edges = create_edge_list(16 * x, 16*y, 16 * x, 16 * (1+y), data->edge_nb);
	else
	{
		if (!(tmp = (t_edge_list*)malloc(sizeof(t_edge_list))))
			return (0);
